...
Node *buildBTFromPostorder(int inorder[], int postorder[], int n, int offset){
    if(n == 0) return NULL;
    int rootVal = postorder[n-1];
    int i = map[rootVal] - offset;
    Node *root = createNode(rootVal);
    root->left = buildBTFromPostorder(inorder, postorder, i, offset);
    root->right = buildBTFromPostorder(inorder+i+1, postorder+i, n-i-1, offset+i+1);
    return root;
}
...
int main(){
    ...
    Node *root = buildBTFromPostorder(inorder, postorder, size, 0);
