...
void serilizationBST(Node *&n, ofstream &o){
    if(!n) o << "#" << " ";
    else {
        o << n->data << " ";
        serilizationBST(n->left, o);
        serilizationBST(n->right, o);
    }

}
int readNextToken(int &token, ifstream &in, bool &isNumber){
    char *s = new char;
    in >> s;
    if(strcmp(s, "#") == 0) {
        isNumber = false;
...
void deserilizationBST(Node *&n, ifstream &in){
    int token = 0;
    bool isNumber;
    ...
    if(isNumber) {
        n = new Node(token);
        deserilizationBST(n->left, in);
        deserilizationBST(n->right, in);
    }
}
...
int main(){
    ...
    serilizationBST(root, ofile);
    ofile.close();
    ifstream in("test.txt");
    Node *newRoot = NULL;
    deserilizationBST(newRoot, in);
    printPretty(newRoot, 1, 5, cout);
    in.close();
    return 0;
}
