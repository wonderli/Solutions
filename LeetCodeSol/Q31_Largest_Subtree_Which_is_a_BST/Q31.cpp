...
int findLargestSubtree(Node *p, int &min, int &max, int &maxNodes, Node *&LargestBST){
    if(!p) return 0;
    bool isBST = true;
    int leftNodes = findLargestSubtree(p->left, min, max, maxNodes, LargestBST);
    int currMin = (leftNodes == 0)? p->data : min;
    if(leftNodes == -1 || leftNodes != 0 && p->data < max){
        isBST = false;
    }
    int rightNodes = findLargestSubtree(p->right, min, max, maxNodes, LargestBST);
...
int main(int argc, char *argv[]){
    ...
    int max = INT_MIN;
    int maxNodes = -1;
    int x = findLargestSubtree(root, min, max, maxNodes, r);
    printPretty(root, 1, 5, cout);
    printPretty(r, 1, 5, cout);
